...
legend longer than the decision, and a room that nods and does not engage. **The whole model is not
an artefact. It is a source from which artefacts are cut.** Completeness feels like the achievement,
...
1. **Name the question in task vocabulary.** Adjacency ("what connects to this?"), accessibility
   ...
   feeling, not a task. Surveys of ontology tooling find it usually remains vague which task a
   visualisation supports, so the habit is endemic, not personal. A picture that supports every task
   supports none. Then check the cheap answer first: an overview task is a number in a sentence, an
   attribute ranking is a sorted list, an adjacency question about one class is two columns. A
   surprising share of these requests resolve that way (`choosing.md`, `tool-task-abstraction.md`).
...
3. **Pick the exemplar vignette before anything is drawn.** One realistic case followed end to end
   ...
   capture to alert. Keep the complete model as an appendix, a browsable tool or a file. This is the
   highest-value move here and the one most reliably skipped.
4. **Separate schema from instances.** Show the schema as structure; show instances as *data* —
   tables, facets, counts, or a handful of exemplar individuals on a fragment of the schema. Where
   the two must meet, encode population as a node *attribute*, not as extra nodes. VOWL's own
   authors dropped their integrated T-box/A-box view after user study on scalability grounds — a
   six-participant think-aloud study whose influence far exceeds its evidence, but the failure it
   names is a category error, not a layout problem: the twelve-class demo collapses on real data
   because every instance became a node.
5. **Let the question choose the form**, not the tooling default:
   ...
   result is experimental and replicated, the strongest in this area; but treat twenty as a floor,
   since the study's sparsest graphs are denser than most hand-built taxonomies, which pushes the
   crossover later for curated models. **The uncomfortable finding:** over a 205-class ontology, a
   plain indented class browser beat four graphical visualisations on both time (about 74 seconds
   against 94, 97 and 190) and correctness (95% against roughly 88%, 88% and 78%). One study, one
   ontology, one task family, tools since discontinued — but it is the most useful thing to put in
   front of anyone insisting their taxonomy needs a diagram, and the list's advantages are
   structural: searchable, collapsible, screen-readable, diffable, copy-pasteable.
6. **Run the leave-it-out list, in order.** (a) Anything that cannot help answer the named question.
   (b) The instances. (c) Everything below the top two or three levels of the hierarchy — depth is
   list territory. (d) `owl:Thing`, reified plumbing, upper-ontology scaffolding, anything named
   `*Abstract*`, unless the audience maintains it. (e) Full expressivity — restrictions,
   cardinalities, unions — for any audience that is not logicians. Keep leaf detail in exactly one
   place: the vignette. Then **caption what you removed and on what rule** — the step that turns a
   reduction into an argument rather than a misrepresentation.
...
8. **Ask domain experts to correct, not approve.** "Does this look right?" buys a yes that validates
   ...
   goal is that they *learn* the domain, have them build the map rather than reading yours: here the
   evidence is genuinely strong — two meta-analyses, and constructing (g = 0.72) clearly beats
   studying (g = 0.43).
...
- [ ] **Question named as a task**, in writing, before any tool is opened — and put it in the caption.
...
- [ ] **Formal notation treated as an audience filter.** UML, ER and ArchiMate carry information a
      reader recovers only by knowing the notation, and Moody's analysis finds such notations
      violate graphic economy and perceptual discriminability (influential, partly contested — later
      work found no advantage for notation improvements in model *construction*). Precision that
      excludes the audience you need is not precision.
...
`reducing-a-large-graph.md`, `naming-the-question-first.md` and `concept-maps.md` in the same
cluster.
